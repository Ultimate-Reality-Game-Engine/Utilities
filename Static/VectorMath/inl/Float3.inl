...
namespace UltReality::Math
{
	...
	{
		return Vector::LoadAFloat3(this);
	}
...
#pragma float_control(pop)
		...
		FORCE_INLINE VECTOR VEC_CALLCONV ReciprocalLength(A_VECTOR v) noexcept
		{
			...
			return Vector::ReciprocalSqrt(Result);
		...
		FORCE_INLINE VECTOR VEC_CALLCONV Orthogonal(A_VECTOR v) noexcept
		{
			...
			VECTOR yzyy = Vector::Swizzle<SWIZZLE_Y, SWIZZLE_Z, SWIZZLE_Y, SWIZZLE_Y>(v);

			VECTOR negativeV = Vector::Subtract(zero, v);

			VECTOR zIsNegative = Vector::Less(z, zero);
			VECTOR yzyyIsNegative = Vector::Less(yzyy, zero);

			VECTOR s = Vector::Add(yzyy, z);
			VECTOR d = Vector::Subtract(yzyy, z);

			VECTOR select = Vector::EqualInt(zIsNegative, yzyyIsNegative);

			VECTOR R0 = Vector::Permute<PERMUTE_1X, PERMUTE_0X, PERMUTE_0X, PERMUTE_0X>(negativeV, s);
			VECTOR R1 = Vector::Permute<PERMUTE_1X, PERMUTE_0X, PERMUTE_0X, PERMUTE_0X>(v, d);
		...
		FORCE_INLINE void VEC_CALLCONV ComponentsFromNormal(VECTOR* pParallel, VECTOR* pPerpendicular, A_VECTOR v, A_VECTOR normal) noexcept
		{
			...
			VECTOR parallel = Vector::Multiply(normal, scale);
		...
		{
			VECTOR A = Vector::Select(g_Select1110.v, v, g_Select1110.v);
			VECTOR Q = Quaternion::Conjugate(rotationQuaternion);
		...
		{
			VECTOR A = Vector::Select(g_Select1110.v, v, g_Select1110.v);
			VECTOR Result = Quaternion::Multiply(rotationQuaternion, A);
			VECTOR Q = Quaternion::Conjugate(rotationQuaternion);
		...
		FORCE_INLINE VECTOR VEC_CALLCONV Transform(A_VECTOR v, A_MATRIX m) noexcept
		{
			...
			VECTOR Result = Vector::MultiplyAdd(Z, m.r[2], m.r[3]);
			Result = Vector::MultiplyAdd(Y, m.r[1], Result);
...
#pragma prefast(disable : 26015 26019, "PREfast noise: Esp:1307" )
		...
		FORCE_INLINE Float4* VEC_CALLCONV TransformStream(Float4* pOutputStream, size_t outputStride, const Float3* pInputStream, size_t inputStride, size_t vectorCount, A_MATRIX m) noexcept
		{
			...
			for (size_t i = 0; i < vectorCount; i++)
			{
				...
				VECTOR Result = Vector::MultiplyAdd(Z, row2, row3);
...
#pragma prefast(disable : 26015 26019, "PREfast noise: Esp:1307" )
		...
		FORCE_INLINE Float3* VEC_CALLCONV TransformCoordStream(Float3* pOutputStream, size_t outputStride, const Float3* pInputStream, size_t inputStride, size_t vectorCount, A_MATRIX m) noexcept
		{
			...
			for (size_t i = 0; i < vectorCount; i++)
			{
				...
				Vector::StoreFloat3(reinterpret_cast<Float3*>(pOutputVector), Result);
				...
				if (inputStride == sizeof(Float3))
				{
					...
					else
					{
						...
						for (size_t j = 0; j < four; ++j)
						{
							...
							Vector::StoreFloat3(reinterpret_cast<Float3*>(pOutputVector), vTemp);
							pOutputVector += outputStride;
...
#pragma prefast(pop)
		...
		FORCE_INLINE VECTOR VEC_CALLCONV TransformNormal(A_VECTOR v, A_MATRIX m) noexcept
		{
			...
			VECTOR Result = Vector::Multiply(Z, m.r[2]);
			Result = Vector::MultiplyAdd(Y, m.r[1], Result);
			Result = Vector::MultiplyAdd(X, m.r[0], Result);
...
#pragma prefast(disable : 26015 26019, "PREfast noise: Esp:1307" )
		...
		FORCE_INLINE Float3* VEC_CALLCONV TransformNormalStream(Float3* pOutputStream, size_t outputStride, const Float3* pInputStream, size_t inputStride, size_t vectorCount, A_MATRIX m) noexcept
		{
			...
			for (size_t i = 0; i < vectorCount; i++)
			{
				...
				VECTOR Result = Vector::Multiply(Z, row2);
				Result = Vector::MultiplyAdd(Y, row1, Result);
				Result = Vector::MultiplyAdd(X, row0, Result);
...
#pragma prefast(pop)
			...
			A_MATRIX projection, B_MATRIX view, B_MATRIX world) noexcept
		{
			...
			VECTOR scale = Vector::Set(halfViewportWidth, -halfViewportHeight, viewportMaxZ - viewportMinZ, 0.0f);
			VECTOR offset = Vector::Set(viewportX + halfViewportWidth, viewportY + halfViewportHeight, viewportMinZ, 0.0f);
			...
			Result = Vector::MultiplyAdd(Result, scale, offset);
...
#pragma prefast(disable : 26015 26019, "PREfast noise: Esp:1307" )
		...
		_Use_decl_annotations_
		FORCE_INLINE Float3* VEC_CALLCONV ProjectStream(Float3* pOutputStream, 
			size_t outputStride, 
			...
			A_MATRIX projection, B_MATRIX view, B_MATRIX world) noexcept
		{
			...
			VECTOR scale = Vector::Set(halfViewportWidth, -halfViewportHeight, viewportMaxZ - viewportMinZ, 1.0f);
			VECTOR offset = Vector::Set(viewportX + halfViewportWidth, viewportY + halfViewportHeight, viewportMinZ, 0.0f);
...
#pragma prefast(pop)
			...
			A_MATRIX projection, B_MATRIX view, B_MATRIX world) noexcept
		{
			...
			scale = Vector::Reciprocal(scale);

			VECTOR offset = Vector::Set(-viewportX, -viewportY, -viewportMinZ, 0.0f);
			offset = Vector::MultiplyAdd(scale, offset, D.v);

			MATRIX transform = Matrix::Multiply(world, view);
			transform = Matrix::Multiply(transform, projection);
			transform = Matrix::Inverse(nullptr, transform);

			VECTOR Result = Vector::MultiplyAdd(v, scale, offset);
