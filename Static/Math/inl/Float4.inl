...
#pragma float_control(pop)
		...
		{
#if defined(DEBUG) || defined(_DEBUG)
			assert((VEC::GetY(lengthMin) == VEC::GetX(lengthMin)) && (VEC::GetZ(lengthMin) == VEC::GetX(lengthMin)) && (VEC::GetW(lengthMin) == VEC::GetX(lengthMin)));
			assert((VEC::GetY(lengthMax) == VEC::GetX(lengthMax)) && (VEC::GetZ(lengthMax) == VEC::GetX(lengthMax)) && (VEC::GetW(lengthMax) == VEC::GetX(lengthMax)));
			...
			VECTOR Result = VEC::(normal, clampLength);
		...
		FORCE_INLINE VECTOR VEC_CALLCONV Reflect(A_VECTOR incident, A_VECTOR normal) noexcept
		{
			...
			return VEC::NegativeMultiplySubtract(Result, normal, incident);
		}
			...
			A_MATRIX m) noexcept
		{
			...
			{
				VECTOR V = VEC::LoadFloat4(reinterpret_cast<const Float4*>(pInputVector));
				VECTOR W = VEC::SplatW(v);
				VECTOR Z = VEC::SplatZ(v);
				VECTOR Y = VEC::SplatY(v);
				VECTOR X = VEC::SplatX(v);

				VECTOR Result = VEC::Multiply(W, row3);
				Result = VEC::MultiplyAdd(Z, row2, Result);
