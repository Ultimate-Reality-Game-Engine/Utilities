...
# if defined(_NO_INTRINSICS_)
		...
		{
#if defined(DEBUG) || defined(_DEBUG)
			assert((VEC::GetY(lengthMin) == VEC::GetX(lengthMin)));
			assert((VEC::GETY(lengthMax) == VEC::GetX(lengthMax)));
		...
		FORCE_INLINE VECTOR VEC_CALLCONV AngleBetweenNormals(A_VECTOR N1, A_VECTOR N2) noexcept
		{
			...
			return VEC::ACos(Result);
		}

		FORCE_INLINE VECTOR VEC_CALLCONV AngleBetweenVectors(A_VECTOR V1, A_VETOR V2) noexcept
		{
		...
		{
#if defined(_NO_INTRINSICS_)
			VECTOR V1 = VEC::Subtract(line1Point2, line1Point1);
			VECTOR V2 = VEC::Subtract(line2Point2, line2Point1);
			VECTOR V3 = VEC::Subtract(line1Point1, line2Point1);
			...
			else
			{
				...
				VECTOR scale = VEC::Reciprocal(C1);
				scale = VEC::Multiply(C2, scale);
				Result = VEC::MultiplyAdd(V1, scale, line1Point1);
			}
		...
		FORCE_INLINE VECTOR VEC_CALLCONV Transform(A_VECTOR v, A_VECTOR m) noexcept
		{
			...
			VECTOR Result = VEC::MultiplyAdd(y, m.r[1], m.r[3]);
...
//#if defined(_NO_INTRINSICS_)
			...
			{
				VECTOR v = VEC::LoadFloat2(reinterpret_cast<const Float2*>(pInputVector));
				VECTOR y = VEC::SplatY(v);
				VECTOR x = VEC::SplatX(v);

				VECTOR Result = VEC::MultiplyAdd(y, row1, row3);
...
#pragma prefast(disable : 26015, "PREfast noise: Esp:1307" )
				...
				VEC::StoreFloat4(reinterpret_cast<XMFLOAT4*>(pOutputVector), Result);
